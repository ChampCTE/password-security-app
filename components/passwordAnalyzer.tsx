...
// SVG Icon Components
const ErrorIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style={{ width: "20px", height: "20px", display: "inline", marginRight: "8px", verticalAlign: "middle" }}>
...
const InfoIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style={{ width: "20px", height: "20px", display: "inline", marginRight: "8px", verticalAlign: "middle" }}>
...
const ShowIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style={{ width: "20px", height: "20px", display: "inline", verticalAlign: "middle" }}>
...
export default function PasswordAnalyzer() {
  ...
  const getIssues = (pw: string) => {
    const issues: (string | React.ReactNode)[] = [];

    if (!pw) return issues;

    if (pw.length < 12) {
      issues.push(
        <span key="length"><ErrorIcon /> Too short, use at least 12 characters</span>
      );
    }

    if (!/[a-z]/.test(pw)) {
      issues.push(
        <span key="lowercase"><ErrorIcon /> Add lowercase letters</span>
      );
    }

    if (!/[0-9]/.test(pw)) {
      issues.push(
        <span key="numbers"><ErrorIcon /> Add numbers</span>
      );
    }

    if (!/[^A-Za-z0-9]/.test(pw)) {
      issues.push(
        <span key="symbols"><ErrorIcon /> Add symbols like !@#$% to strengthen security</span>
      );
    }

    if (/(1234|abcd|qwerty|1111|aaaa)/i.test(pw)) {
      issues.push(
        <span key="patterns"><ErrorIcon /> Avoid common patterns like '1234', 'abcd', 'qwerty', '1111', or 'aaaa'</span>
      );
    }
...
/* Advice */
  ...
  const getAdvice = (pw: string) => {
    const advice: (string | React.ReactNode)[] = [];

    if (!pw) return advice;

    if (pw.length < 12) {
      advice.push(
        <span key="length-advice"><InfoIcon /> Use at least 12-16 characters for greater security.</span>
      );
    }

    if (!/[A-Z]/.test(pw)) {
      advice.push(
        <span key="uppercase-advice"><InfoIcon /> Add uppercase letters to increase entropy.</span>
      );
    }

    if (!/[0-9]/.test(pw)) {
      advice.push(
        <span key="numbers-advice"><InfoIcon /> Include numbers for better security.</span>
      );
    }

    if (!/[^A-Za-z0-9]/.test(pw)) {
      advice.push(
        <span key="symbols-advice"><InfoIcon /> Use symbols like !@#$% to strengthen security.</span>
      );
    }

    if (score <= 2) {
      advice.push(
        <span key="vulnerable-advice"><InfoIcon /> Your password is vulnerable to dictionary or brute force attacks.</span>
      );
    }

    if (score === 4) {
      advice.push(
        <span key="strong-advice"><CheckIcon /> Good password, hard to crack.</span>
      );
    }
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style={{ width: "28px", height: "28px", display: "inline", marginRight: "10px", verticalAlign: "middle" }}>
      ...
      <div style={styles.card}>
        ...
        <button onClick={() => setShowPassword(!showPassword)} style={styles.button}>
      ...
      {password && (
        <div style={styles.card}>
          <p>
            Level: <strong>{getLabel(score)}</strong>
          </p>
          ...
          <p>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style={{ width: "16px", height: "16px", display: "inline", marginRight: "6px", verticalAlign: "middle" }}>
